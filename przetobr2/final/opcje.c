...
typedef struct {
  ...
  int negatyw,progczerni,konturowanie,wyswietlenie,rozhist;      /* opcje */
  int w_progu;              /* wartosc progu dla opcji progczerni */ 
} w_opcje;
...
/*	poprawnie wymytapily w linii wywolania programu,                 */
/*	pola opcji nie wymytepujacych w wywolaniu ustawione sa na 0;     */
...
int przetwarzaj_opcje(int argc, char **argv, w_opcje *wybor) {
  ...
  for (i=1; i<argc; i++) {
    ...
    case 'h': {                 /* mamy rozciagnac histogram */
      wybor->rozhist=1;
      break;
    }
